...
CONTAINS
  ...
  SUBROUTINE W3SDB1 (IX, A, DEPTH, EMEAN, FMEAN, WNMEAN, CG, LBREAK, S, D )
    ...
    ELSE IF (IWB == 2) THEN
      IF (ETOT .GT. THR) THEN
        HRMS = SQRT(8.D0*EMEAN)
        FAK  = (1.D0+4.D0/SQRT(PI)*(B*BB+1.5D0*B)*exp(-BB)-ERF(B))
        CBJ  = -SDBC1*SQRT(PI)/16.D0*FMEAN*HRMS**3/DEPTH/ETOT
      ELSE
        CBJ  = 0.D0
      ENDIF
      D = real(- CBJ, 4)
...
9000 FORMAT (' TEST W3SDB1 : PARAMETERS :',2F7.3,L4)
  ...
  SUBROUTINE W3SDB1double(IX, A, DEPTH, EMEAN, FMEAN, WNMEAN, CG, LBREAK, S, D )
    !/
    !/                  +-----------------------------------+
    !/                  | WAVEWATCH III           NOAA/NCEP |
    !/                  |                        FORTRAN 90 |
    ...
    !/
    INTEGER, INTENT(IN)     :: IX ! Local grid number
    REAL*8, INTENT(IN)        :: A(NSPEC)
    REAL*8, INTENT(INOUT)     :: EMEAN, FMEAN, WNMEAN, DEPTH
    REAL*8, INTENT(OUT)       :: S(NSPEC), D(NSPEC)
    REAL*8, INTENT(IN)        :: CG(NK)
    LOGICAL, INTENT(OUT)    :: LBREAK
    INTEGER                 :: ITH, IK, IWB
    !/
    !/ ------------------------------------------------------------------- /
    ...
    ELSE IF (IWB == 2) THEN
      IF (ETOT .GT. THR) THEN
        HRMS = SQRT(8.D0*EMEAN)
        FAK  = (1.d0+4.d0/SQRT(PI)*(B*BB+1.5*B)*exp(-BB)-ERF(B))
        CBJ  = -SDBC1*SQRT(PI)/16.d0*FMEAN*HRMS**3/DEPTH/ETOT
      ELSE
        CBJ  = 0.d0
      ENDIF
